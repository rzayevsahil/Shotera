...
fn resize_recorder_window(app_handle: AppHandle, compact: bool) -> Result<(), String> {
    ...
    if let Some(window) = app_handle.get_webview_window("recorder") {
        if compact {
            let _ = window.set_size(tauri::LogicalSize::new(380.0, 110.0));
            if let Ok(Some(monitor)) = window.current_monitor() {
                let size = monitor.size();
                let scale_factor = monitor.scale_factor();
                let x = (size.width as f64 - (400.0 * scale_factor)) as i32;
                let y = (size.height as f64 - (140.0 * scale_factor)) as i32;
