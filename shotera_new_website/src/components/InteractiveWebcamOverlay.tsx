...
export function InteractiveWebcamOverlay({
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      setPosition({
        x: Math.max(12, Math.min(500, dragStartRef.current.startPosX + dx)),
        y: Math.max(12, Math.min(380, dragStartRef.current.startPosY + dy)),
      });
    };
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute -top-10 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-950/90 border border-slate-800 backdrop-blur-md shadow-xl text-slate-300 transition-all duration-200 ${
      ...
      >
        ...
        <button
          type="button"
          onClick={() => {
            const styles: BorderAnimationType[] = ['spin-cyber', 'spin-rainbow', 'spin-ocean', 'spin-fire', 'solid', 'pulse', 'breathe'];
            const nextIdx = (styles.indexOf(borderAnimation) + 1) % styles.length;
            setBorderAnimation(styles[nextIdx]);
          }}
          className="p-1 rounded-full hover:bg-slate-800 text-slate-300 hover:text-white transition-colors"
          title="Border Style Cycle"
        >
          <Sparkles className="w-3.5 h-3.5 text-sky-400" />
        </button>

        {/* Real Camera / Avatar toggle */}
        <button
          type="button"
          onClick={() => {
            if (hasRealCamera) {
          ...
          className={`p-1 rounded-full transition-colors ${
            hasRealCamera ? 'bg-sky-500/20 text-sky-300 hover:bg-sky-500/30' : 'hover:bg-slate-800 text-slate-300 hover:text-white'
          }`}
          title={hasRealCamera ? 'Switch to Presenter Avatar' : 'Connect Real Webcam'}
        >
          {hasRealCamera ? <Video className="w-3.5 h-3.5" /> : <VideoOff className="w-3.5 h-3.5" />}
        </button>
        ...
        <button
          ...
          onClick={() => {
            ...
            const nextIdx = (sizes.indexOf(webcamSize) + 1) % sizes.length;
            setWebcamSize(sizes[nextIdx]);
          }}
          className="px-1.5 py-0.5 rounded text-[10px] font-mono-code font-bold bg-slate-800 hover:bg-slate-700 text-slate-200 transition-colors uppercase"
          title="Resize Webcam"
        >
          {webcamSize}
        </button>
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-1 rounded-full hover:bg-rose-950/60 text-slate-400 hover:text-rose-400 transition-colors"
          title="Close Camera"
        >
        ...
        >
          ...
          {!hasRealCamera && (
            ...
            >
              ...
              {/* Live Status indicator in corner */}
              <div className="absolute bottom-2.5 right-2.5 z-10 flex items-center gap-1 bg-slate-950/80 backdrop-blur-sm px-1.5 py-0.5 rounded-full border border-slate-700/60 text-[9px] font-mono-code text-slate-300">
