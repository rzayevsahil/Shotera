...
- 🎥 **Запись Экрана (`Ctrl` + `5`):** Аппаратно-ускоренная нативная запись экрана с использованием Windows Graphics Capture (WGC/DXGI) и GPU-кодеков (NVENC/QSV/AMF) для создания высококачественного MP4-видео без задержек. Прямой выбор мониторов или окон.
